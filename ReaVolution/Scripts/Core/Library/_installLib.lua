...
elseif os == "macOS-arm64" then
    jsAPI = io.open(reaper.GetResourcePath().."/UserPlugins/reaper_js_ReaScriptAPI64ARM.dylib", "r")
else
    reaper.ShowMessageBox("System is not supported", "Error : unsupported system detected", 0)
    if foo == 1 then reaper.Main_OnCommandEx(40004, 0, 0) end--File: Quit REAPER
end
